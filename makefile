...
JAVA_OPTS=" -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.nexus.nsnik.randomno"
...
$(ESPRESSO_TESTS) : app-androidTest.apk app-instrumented.apk
	...
	$(eval FIRST_RUN := $(shell $(ADB) shell pm list packages | grep -c com.nexus.nsnik.randomno))
	@if [ "$(FIRST_RUN)" == "2" ]; then \
		echo "Resetting the data of the apk"; \
		$(ADB) shell pm clear com.nexus.nsnik.randomno; \
	else \
...
#	Log directly to the expected file
	$(ADB) shell am instrument -w -e class $(TEST_NAME) com.nexus.nsnik.randomno.test/android.support.test.runner.AndroidJUnitRunner 2>&1 | tee $(@)
